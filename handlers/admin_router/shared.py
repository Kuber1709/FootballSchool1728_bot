...
async def workouts_show(message: Message, number: int, group_id: int, delete: bool = False):
    ...
    text = txt.admin.workouts_text.substitute(text=text, method_text=method[0])
...
async def admins_show(message: Message, admin_id: int, delete: bool = False):
    ...
    if delete:
        ...
        result = await message.answer(text, reply_markup=kb.admin.inline.admin(admin_id), parse_mode="Markdown")
        result_inline = None
